...
public class PatientServiceTest {
    ...
    @Transactional
    @Test
    public void ShouldReturnPatientVisit(){
        var PatientId = 1L;
        var patient = patientDao.findOne(PatientId);
        var patientVisit = patient.getVisits();
        var visits = patientService.findVisitsByPatientId(PatientId);
        assertThat(visits).isNotNull();
        assertEquals(visits.size(), patientVisit.size());
        assertEquals(visits, patientVisit);
    }
}
